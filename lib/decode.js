  ...
  if (Array.isArray(decodedDeeplink.p)) {
    ...
    Object.keys(decodedDeeplink.p).forEach(function (key) {
      params[decodeURIComponent(key)] = [{
        value: decodeURIComponent(decodedDeeplink.p[key][0].v)
      }];
    });
  }

  if (Array.isArray(decodedDeeplink.tg)) {
    tags = decodedDeeplink.tg.map(function (tg) {
      return {
        label: decodeURIComponent(tg.l),
        value: decodeURIComponent(tg.v)
      };
    });
  } else if (decodedDeeplink.tg && _typeof(decodedDeeplink.tg) === 'object') {
    tags = {};
    Object.keys(decodedDeeplink.tg).forEach(function (key) {
      tags[decodeURIComponent(key)] = [{
        value: decodeURIComponent(decodedDeeplink.tg[key][0].v)
      }];
    });
  }

...
var encode = function encode(params) {
  ...
  if (Array.isArray(params.params)) {
    ...
    Object.keys(params.params).forEach(function (key) {
      deeplinkParams.p[encodeURIComponent(key)] = [{
        v: encodeURIComponent(params.params[key][0].value)
      }];
    });
  }

  if (Array.isArray(params.tags)) {
    deeplinkParams.tg = params.tags.map(function (param) {
      return {
        l: encodeURIComponent(param.label),
        v: encodeURIComponent(param.value)
      };
    });
  } else if (params.tags && _typeof(params.tags) === 'object') {
    deeplinkParams.tg = {};
    Object.keys(params.tags).forEach(function (key) {
      deeplinkParams.tg[encodeURIComponent(key)] = [{
        v: encodeURIComponent(params.tags[key][0].value)
      }];
    });
  }
